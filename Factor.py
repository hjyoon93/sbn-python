...
class Factor:
    ...
    def reorder(self, observed_nodes):
        ...
        dtype = [(str(i), bool) for i in range(self.nodes.size)] + [('prob', np.float64)]
    ...
    def rebuild_factor(self, num_common_nodes_total_combinations):
        ...
        return np.array(products).astype(np.float64)
    ...
    def marginalization(self, node):
        ...
        sums = np.array(sums_list).astype(np.float64)
    ...
    def normalize(self, query):
        ...
        prob_column = self.prob[:,-1].astype(np.float64).reshape((-1, 1))
